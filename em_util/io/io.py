...
def read_nd2(filename, dataset=None):
    """
    Read data from a nd2 file.

    Args:
        filename (str): The path to the nd2 file.
        dataset (int, optional): The frame id of the dataset within the file. Defaults to -1 for all frames.
    ...
    if isinstance(dataset, int):
        assert(dataset < len(frames), f'{dataset}-th frame is out of bound. #frames={len(frames)}')
        data = np.array(frames[dataset])
    else:
        if dataset is None:
            dataset = range(len(frames))
        assert(np.max(dataset) < len(frames), f'{np.max(dataset)}-th frame is out of bound. #frames={len(frames)}')
        data = [np.array(frames[x]) for x in dataset]
    
    return data
 
def read_zarr(filename, dataset=None):
    """
