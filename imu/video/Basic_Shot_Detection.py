...
class Shot_detect:
    ...
    def clip_peak(
        self, list, clipping_thresh=0.4, window_length=20, stride=10
    ):
        i = 0
    ...
    def Gaussian_mask(self, c1, c2, v1, v2):
        ...
        for x in range(self.att.shape[0]):
            for y in range(self.att.shape[1]):
                a = math.exp(
                    -((x - c1) ** 2 / (360 * v1) + (x - c2) ** 2 / (640 * v2))
                )
                self.att[x, y, 0], self.att[x, y, 1], self.att[x, y, 2] = (
                    a,
                    a,
                    a,
                )
    ...
    def thresh_image(self, frame_id):
        ...
        """
        assert (
            self.hsv_high is not None and self.hsv_high_tab is not None
        ), "Cannot segment image!! Please set HSV thresholds first."
        im = cv2.imread(self.fns[frame_id])
        im_hsv = cv2.cvtColor(
            im[
                self.crop_indices[0] : self.crop_indices[1],
                self.crop_indices[2] : self.crop_indices[3],
                :,
            ],
            cv2.COLOR_BGR2HSV,
        )
    ...
    def extract_features(self, save_path=None):
        assert (
            self.hsv_high is not None and self.hsv_high_tab is not None
        ), "Cannot extract features!! Please set HSV thresholds first."
        assert (
            self.crop_indices is not None
        ), "Cannot extract features!! Please set cropping indices first."
    ...
    def detect_shots(
        ...
        assert (
            self.feat is not None
        ), "No features found!! Please extract features first."
    ...
    def generate(self):
        ...
        shot_label = np.vstack(
            [np.ones([1, num]), np.zeros([1, num])]
        ).T.reshape(-1)
        saveShotDetection(
            os.path.join(self.video, "shot.js"), self.shot_start, shot_label
        )
        video_name = self.video[self.video[:-1].rfind("/") + 1 :]
        out = html_shot(
            video_name,
            frame_name=video_name + "/%06d.jpg",
            file_result=video_name + "/shot.js",
            frame_fps=1,
        )
        write_txt(
            os.path.join(
                os.path.dirname(self.video),
                "shot_detection_" + video_name + ".html",
            ),
            out.getHtml(),
        )
