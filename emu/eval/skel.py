...
class NodeAssignment:
    ...
    def get_id_maps(self):
        ...
        pred_ids = (
            pred_ids[size_order].astype(int).tolist()
        )  # sort in descending order of intersection
...
class SkeletonEvaluation:
    ...
    def __init__(
        ...
        def merge_check(
            id_map1, id_map2, t_m=t_m, include_zero_merge=include_zero_merge
        ):
            """
            ...
            top_ids1 = [
                id_map1.ids_out[i]
                for i in range(len(id_map1.ids_out))
                if (1.0 * id_map1.counts_out[i]) / id_map1.size_in > t_m
            ]
            top_ids2 = [
                id_map2.ids_out[i]
                for i in range(len(id_map2.ids_out))
                if (1.0 * id_map2.counts_out[i]) / id_map2.size_in > t_m
            ]
        ...
        def split_check(
            skeleton, id_map, include_zero_split=include_zero_split, t_s=t_s
        ):
            """
            ...
            if 0 in pred_ids and not include_zero_split:
                ...
                pred_ids.remove(0)
            if (1.0 * pred_counts[0]) / id_map.size_in > t_s:
                return [pred_ids[0]]
            else:
                return pred_ids
        ...
        try:
            ...
            print(
                "Predicted segmentation and GT skeleton volume have different grid size"
            )
        self.grid_size = (pred.shape[0], pred.shape[1], pred.shape[2])
        print(
            "Starting evaluation of %d labels in %dx%dx%d predicted segmentation against %d GT skeletons"
            % (self.n_preds, pred.shape[0], pred.shape[1], pred.shape[2], self.n_skels)
        )
        # error thresholds
        self.t_om = t_om
        self.t_m = t_m
        self.t_s = t_s
        print(
            "Using error thresholds: t_om=%.2f, t_m=%.2f, t_s=%.2f" % (t_om, t_m, t_s)
        )
        # main results
        ...
        # GT to pred ID maps
        self.gt2pred = [
            NodeAssignment(gt_skeletons[i], pred).get_id_maps()
            for i in range(self.n_skels)
        ]
        ...
        for i, sk_outer in enumerate(gt_skeletons):
            ...
            # case: omitted
            omission_flag = (
                self.gt2pred[i].ids_out[0] == 0
                and (1.0 * self.gt2pred[i].counts_out[0]) / self.gt2pred[i].size_in
                > self.t_om
            )
            if omission_flag:
                self.categories[i] = "omitted"
            ...
            if len(split_ids) > 1:
                if self.categories[i] == "merged":
                    self.categories[i] = "hybrid"
                else:
        ...
        if calc_erl == True:
            ...
            for i, sk in enumerate(gt_skeletons):
                ...
                if category is "correct":
                    ...
                    pred_length += (split_frac * split_frac) * (
    ...
    def summary(self, write_path=None):
        ...
        print(
            "Results:\n%d omissions, %d merges, %d splits, %d hybrid, %d correct"
            % (n_om, n_m, n_s, n_h, n_c)
        )
        print("GT ERL: %d, Prediction ERL: %d" % (self.erl_gt, self.erl_pred))
        print("GT TRL: %d, Prediction TRL: %d" % (self.trl_gt, self.trl_pred))
        print(
            "Omitted RL: %d, Merged RL: %d, Split RL: %d"
            % (self.omitted_length, self.merged_length, self.split_length)
        )
        if write_path is not None:
            cat_counts = {
                "thresholds": {"t_om": self.t_om, "t_m": self.t_m, "t_s": self.t_s},
                "results": {
                    "total": self.n_skels,
                    "omitted": n_om,
                    "merged": n_m,
    ...
    def write_errors(self, write_path):
        ...
        for merge_item in self.merge_list:
            ...
            f.write(
                "%d, %d\n" % (merge_item.ids_out[0], merge_item.ids_out[1])
            )  # IDs of merged skeletons
        # write splits
        ...
        for corr_item in self.corr_list:
            f.write(
                "%d, %d\n" % (corr_item.id_in, corr_item.ids_out[0])
            )  # GT id, pred id
